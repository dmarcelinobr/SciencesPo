...
### d'Hondt
Under the D'Hondt formula, seats are allocated using divisors of 1, 2, 3, 4, etc.
...
The d'Hondt method is only one way of allocating seats in party list systems. Other methods include the Saint-Laguë, the modified Saint-Laguë, the Danish version, Imperiali (do not to confuse with the Imperiali quota, which is a Largest remainder method), and Hungtinton-Hill. 

### Modified Saint-Laguë
How does d'Hondt compare to modified Sainte-Laguë? The modified Sainte-Laguë use divisors of 1.4, 3, 5, 7, etc.
...
### Saint-Laguë
In this setting, the pure Sainte-Laguë formula, which uses the odd-integer divisor series 1, 3, 5, 7, and so forth, would produce the same outcome.
...
### Hungtinton-Hill
The method of proposed by Joseph Hill and Edward Huntington do not use standard divisor, but a different divisor and modified quota such that rounding with cut-off point at geometric mean: hh = $\sqrt{L}{(L + 1)}$.
...
### Webster
This approach do not use standard divisor either. Instead, it uses a different divisor and modified quota such that rounding with cut-off point at algebraic mean  of lower and upper quotas: hh = $\frac{L + (L +1)}{2}$.

```{r highestAverages6, eval=FALSE, echo=TRUE, message=FALSE, comment=NA}
highestAverages(parties=names(lijphart), votes=lijphart, 
                seats = 6, method = "wb") 
```
...
Highest averages methods allocate seats proportionally to the number of votes by dividing the number of votes required for each party by a quota. The quota may vary because it is the result of dividing the number of unspoiled votes ($v$) by some whole number close to the number of seats that would next be assigned ($s$). In what follows are llustrative examples of the operation of three largest remainders formulas in an eight member district with the same four parties of the previous examples.
...
### Imperiali quota
The imperial quota is obtained by quota = $\frac{v}{(s + 2)}$.
