...
```{r politicalDiversity1, echo=FALSE, message=FALSE}
library("SciencesPo")
...
```{r Helsinki-election, eval=TRUE, echo=FALSE, message=FALSE}
# Helsinki's 1999
...
```{r highestAverages1, echo=TRUE, message=FALSE}
...
## Now let's consider some of the largest remainder methods
...
```
 ...
 Next, using the largest remainder method with the Imperiali quota.
...
Because the output of these functions is a `data.frame`, we can use  `knitr` to produce academic style tables as in the following example.
