...
# Overview
The purpose of this vignette is to introduce the **SciencesPo** package to familiarize the reader with basic functions and how these can be used in data analysis workflows and reporting results. The package is meant to provide primarily functions and algorithms for analyzing political behavior data, including measures of political fragmentation and seat allocation methods. However, it also supplies some functions to do descriptive statistics, tests, cross-tables, and rendering pre set publication-quality plots that will require only a minimum amount of fiddling details. Although this package is available for the general public, it meets my personal needs and tastes. Yours may be different.

The development of this package began in July of 2011. I found myself spending a great deal of my time manipulating several datasets with a basket of functions, but only a very little time actually analyzing data. So, I thought in a more efficient way of doing recurrent tasks. Since then, I have diligently labored to maintain this R package for social data analysis and plotting. The **SciencesPo** is currently hosted by the R-CRAN Foundation and distributed over the internet. You can find the package source on [Github](https://github.com/danielmarcelino/SciencesPo), and you are welcome to contribute code via pull requests, or bug reports.
...
The **SciencesPo** brings in few themes and color palettes to enhance  **ggplot2** graphs, but there are specialized packages like **ggthemes**, **cowplot**, and **** that can make graph decoration a lot easier.
...
```{r eval=FALSE, echo=TRUE, fig.width=4.5, fig.height=4.5}
