...
#' print(m)
#'
#' uncertainty(m); # default is direction = "symmetric"
#'
#' uncertainty(m, conf.level=0.95); # direction "symmetric"
#'
#' uncertainty(m, direction="column");
#' }
#' @export
 `uncertainty` <- function(x, y = NULL, direction = c("symmetric", "row", "column"), conf.level = NA, p.zero.correction = 1/sum(x)^2, ...) UseMethod("uncertainty")

`uncertainty.default` <- function(x, y = NULL, direction = c("symmetric", "row", "column"), conf.level = NA, p.zero.correction = 1/sum(x)^2, ... ) {
  # Theil's UC (1970)
