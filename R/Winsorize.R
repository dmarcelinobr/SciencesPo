...
#'
#' @description Compute the winsorized mean, which consists of recoding the top k values of a vector.
#'
...
#'
#' @details Winsorizing a vector will produce different results than trimming it. While trimming a vector causes the extreme values to be discarded, winsorizing causes the extreme values to be replaced by certain percentiles.
#'
...
`Winsorize` <-
  function (x, k = 1, na.rm=TRUE) {
    ...
    if (!(k %in% (0:n)))
      ...
      x <- sort(x)
      x[1:k] <- x[k+1] # Solve lower values
      x[(n-k+1):n] <- x[n-k] #Then go over the higher ones
      return(Mean(x))
    }
  }### end -- WinsorizedMean function
NULL
