...
detail <-
  function(data, by = NULL, basic = TRUE, na.rm = TRUE, trim = 0.2, type = 2, k = 1)
  {
    ...
    if (is.null(dim(data)[2])) {
      ...
      stats[1, 3] <- sd(data, na.rm = na.rm)
      stats[1, 4] <- var(data, na.rm = na.rm)
      stats[1, 5] <- se(data, na.rm = na.rm)
      stats[1, 6] <- median(data, na.rm = na.rm)
      ...
      stats[, 3] <- sapply(data, FUN=sd, na.rm = na.rm)
      stats[, 4] <- sapply(data, FUN=var, na.rm = na.rm)
      stats[, 5] <- sapply(data, FUN=se, na.rm = na.rm)
      stats[, 6] <- sapply(data, FUN=median, na.rm = na.rm)
      stats[, 7] <- sapply(data, FUN=mad, na.rm = na.rm)
