...
#' 
#' @description Yield for monthly aggregation. It does handle discontinuous time series by months. The user can also set its own method by passing an ad-hoc function for the aggregation method.
#'
#' @param data A time-series object
#' @param FUN A function to aggregate data
#' @param months A numeric value for the number of months to perform aggregation
...
#' 
#' @seealso \link{by_day}, \link{by_year}
#' 
...
#'  monthly <-by_month(Obama.ts, mean)
#' # bimonthly or semimonthly aggregated means for data:
#' bimonthly <-by_month(Obama.ts, mean, 2) }
#'
#' @export
#' 
by_month <-
