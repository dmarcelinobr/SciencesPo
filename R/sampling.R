...
#' @export
#' @examples
#' sampleCircular(500, 30)
`sampleCircular` <- function(N = 500, n = 30) {
  k = round(N / n, 0)
  r = sample(1:N, 1)
  f = n - 1
  circ.syst = r
...
#' sampleLinear(500, 30)
#'
`sampleLinear` <- function(N = 500, n = 30) {
  k = round(N / n, 0)
  r = sample(1:k, 1)
...
#' @export
#' @examples
#' sampleReplicated(500, 30, 6)
#'
`sampleReplicated` = function(N = 500,
                              n = 30,
                              g = 6) {
  m = n / g
  samp = sample(1:N, n)
...
#'
#' @examples
#' sampleLinearReplicated(500, 30, 6)
#' @export
`sampleLinearReplicated` <- function(N = 500,
                                     n = 30,
                                     g = 6) {
  m = round(n / g, 0)
  k = round(N / m, 0)
...
#'
#' @examples
#' sampleCircularReplicated(500, 30, 6)
#' @export
`sampleCircularReplicated` <- function(N = 500,
                                       n = 30,
                                       g = 6) {
  m = round(n / g, 0)
  k = round(N / m, 0)
...
`samplePower` <- function(mu0 = 0,
  ...
  if (mu1 != mu0) {
    ...
    cat('      power =', round(1 - stats::pnorm(
      stats::qnorm(1 - alpha / 2, mean = 0, sd = se.mean),
      mean = abs(mu1 - mu0),
      sd = se.mean
    ), 3), '\n')
    cat('alternative = two-sided\n\n')
    if (n < 30)
      cat('Warning: N too small for z test!\n\n')
  }

  # Normal curve H0:

  graphics::curve(
    stats::dnorm(x, mean = mu0, sd = se.mean),
    from = z0min,
    to = z0max,
    xlim = range(z0min, z0max, z1min, z1max),
    ylab = 'density',
    col = "blue",
    xlab = paste(
      '\u03B1 = ',
    ...
    main = paste(
      'Power of z-Test of the Mean of a Single Population\n',
      'n = ',
      n,
      ', \u03BC0 = ',
      mu0,
  ...
  graphics::polygon(c(z0L, x, z0U), c(0, stats::dnorm(x, mean = mu0, sd =
                                                        se.mean), 0), col = "lightyellow")

  # Normal curve H1:
  graphics::curve(
    stats::dnorm(x, mean = mu1, sd = se.mean),
    from = z1min,
    to = z1max,
    add = TRUE,
    col = "red"
  ...
  if (mu1 > mu0) {
    ...
    graphics::polygon(c(z0U, x, z1max), c(0, stats::dnorm(x, mean = mu1, sd =
                                                            se.mean), 0),
                      col = "lightyellow")

    # Rejection region H0:
    x = seq(z0U, z0max, min(.001, 1 / n))
    graphics::polygon(c(z0U, x, z0max),
                      c(0, stats::dnorm(x, mean = mu0, sd = se.mean), 0),
                      density = 20,
                      col = "blue")
    x = seq(z0min, z0L, min(.001, 1 / n))
    graphics::polygon(c(z0min, x, z0L),
                      c(0, stats::dnorm(x, mean = mu0, sd = se.mean), 0),
                      density = 20,
                      col = "blue")
    ...
    if (z1min < z0U) {
      ...
      graphics::polygon(
        c(z1min, x, z0U),
        c(0, stats::dnorm(x, mean = mu1, sd = se.mean), 0),
        density = 20,
        angle = 135,
        col = "red"
      )
    ...
    graphics::polygon(c(z1min, x, z0L), c(0, stats::dnorm(x, mean = mu1, sd =
                                                            se.mean), 0),
                      col = "lightyellow")
    ...
    cat('Error: \u03BC1 must differ from \u03BC0!\n')
}
NULL
...
`stratified` <- function(.data,
  ...
  if (length(size) > 1) {
    ...
    if (all(.data.table >= size) || isTRUE(replace)) {
      ...
      n <-
        c(sapply(.data.table[.data.table >= size], function(x)
          x = size),
          .data.table[.data.table < size])
    }
  }
  temp <- lapply(names(.data.split),
                 function(x)
                   .data.split[[x]][sample(.data.table[x],
                                           n[x], replace = replace),])
