...
#' plotNorm()
#' plotNorm(below=-1.5)
#' plotNorm(below=-1.5,justbelow=TRUE)
#' plotNorm(above=1.5, justabove=TRUE)
#' plotNorm(below=-1.5,above=1.5)
#' plotNorm(between=c(-4,0),color="black")
#' plotNorm(between=c(0,4),color="black")
#' plotNorm(between=c(-1,+1),color="darkgray")
#' title("P[-1 < z < 1] = 68%")
#' plotNorm(between=c(-2,+2),color="darkgray")
#' title("P[-2 < z < 2] = 95%")
#' plotNorm(between=c(-3,+3),color="darkgray")
#' title("P[-3 < z < 3] = 99.7%")
#' plotNorm(between = c(-1.75, 0, 2, 0.5, -1))  ## Plots between specified points
#' plotNorm(below=50,justbelow=TRUE,color="black",mu=47.3,sigma=9.3)
#'
#' ## Can plot one and then another on top of it using lines = TRUE
#' plotNorm(mu=2, sigma=10, outside=c(-3, 12), dens=15)
#' plotNorm(mu=2, sigma=15, between=c(-3, 12),lines=TRUE, col="blue",dens=15)
...
#' plotNorm(outside = crits, mu = mu.ho, sigma = std.err,dens=15)
