...
#'
#' @description \code{FillIn} uses values of a variable from one data set to fill in missing values in another.
#' 
...
#' @param xvar a character string of the name of the variable in \code{x} you want to fill in.
#' @param yvar an optional character string of variable name in \code{y} that you would like to use to fill in.
#' @param KeyVar a character vector of variable names that are shared by \code{x} and \code{y} that can be used to join the data frames.
...
#' FilledInData <- FillIn(naDF, fillDF, xvar = "fNA", yvar = "fFull", KeyVar = c("a", "b"))
#' @export

FillIn <- function(x, y, xvar, yvar = NULL, KeyVar = c("iso2c", "year"))
{
  # Give yvar the same name as xvar if yvar is NULL
  if (is.null(yvar)){
    yvar <- xvar
  } else {
    yvar <- yvar
  ...
  xTemp <- data.table::data.table(x, key = KeyVar)
  yTemp <- data.table::data.table(y, key = KeyVar)
