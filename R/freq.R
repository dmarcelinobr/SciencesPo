...
#' @param \dots Further arguments are: \code{digits}, \code{\link{cut}()}, and \code{right} to define if the intervals should be closed on the right (and open on the left) or vice versa.
#'
#' @examples
#' data(ssex)
...
#' @export
`freq` <-
  function(x, breaks = hist(x, plot = FALSE)$breaks, include.lowest = TRUE, ord = c("level", "desc", "asc", "name"),
    ...
    if(inherits(x, "table")){
      ...
      if(is.numeric(x)){
        x <- base::cut(x, breaks = breaks, include.lowest = include.lowest,
                 ordered_result = TRUE, ...)
      }
