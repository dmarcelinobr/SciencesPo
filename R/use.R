...
#' @param dataFrame
#' @param clear
#' @param spss.missing , default is \code{spss.missing = TRUE}.
#' @param tolower  , default is \code{tolower = TRUE}.
#' 
...
"use" <-
  function (file, dataFrame = .data, clear = TRUE, spss.missing = TRUE, tolower = TRUE) 
  {
    if (clear) {
      detachAll()
    }
