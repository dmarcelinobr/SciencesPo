...
#'
#' @param ref File reference, either omitted to browse for the data file, or (except for .Rda files) a full path name or web URL, included in quotes.  A URL begins with \code{http://}.
...
#'
`use` <-
function(ref=NULL, format=c("csv", "SPSS", "R", "Excel", "SAS"),
...
# option to browse for data file, and then display file name
  ...
  if (!is.null(labels)) {  # process labels
    if (format %in% c("fwd", "csv", "Excel")) {
      if (labels != "row2") {  # read labels file
        ...
        if (format.lbl != "Excel")
          ...
          mylabels <- readxl::read_excel(path=ref.lbl, col_names=FALSE)
        ...
        if (format != "Excel")
          ...
          mylabels <- readxl::read_excel(path=ref, ...)
          mylabels <- mylabels[1,]
        }
        ...
        if (format != "Excel")
          ...
          d <- readxl::read_excel(path=ref, col_names=var.names)
        }
    ...
    d <- foreign::read.spss(file=ref, to.data.frame=TRUE, ...)

  else if (format == "SAS") { # data
    d <- sas7bdat::read.sas7bdat(file=ref, ...)
